import { getSession, isSuperAdmin, requireTenant } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";
import { notFound, redirect } from "next/navigation";
import { NewsEditor } from "../new/news-editor";

interface Props {
  params: Promise<{ id: string; newsId: string }>;
}

export default async function EditNewsPage({ params }: Props) {
  const { id, newsId } = await params;
  const session = await getSession();

  if (!session) {
    redirect("/");
  }

  try {
    await requireTenant(id);
  } catch {
    notFound();
  }

  const canEdit =
    isSuperAdmin(session) ||
    (session.user.tenantId === id &&
      ["ADMIN", "EDITOR"].includes(session.user.role));

  if (!canEdit) {
    redirect(`/dashboard/tenants/${id}/news`);
  }

  const tenant = await prisma.tenant.findUnique({
    where: { id },
  });

  if (!tenant) {
    notFound();
  }

  // Pobierz aktualność
  const news = await prisma.page.findUnique({
    where: { id: newsId },
    select: {
      id: true,
      slug: true,
      title: true,
      description: true,
      content: true,
      status: true,
      publishedAt: true,
    },
  });

  if (!news || news.status === undefined) {
    notFound();
  }

  // Parsuj content
  let content = { html: "", image: "" };
  try {
    content =
      typeof news.content === "string"
        ? JSON.parse(news.content)
        : (news.content as { html: string; image: string });
  } catch {
    content = { html: "", image: "" };
  }

  const initialData = {
    id: news.id,
    title: news.title,
    slug: news.slug,
    description: news.description || "",
    content,
    status: news.status as "DRAFT" | "PUBLISHED" | "ARCHIVED",
    publishedAt: news.publishedAt ? news.publishedAt.toISOString() : null,
  };

  return (
    <div className="max-w-4xl">
      {/* Header */}
      <div className="mb-8">
        <Link
          href={`/dashboard/tenants/${id}/news`}
          className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-4"
        >
          <ArrowLeft className="h-4 w-4" />
          Powrót do aktualności
        </Link>

        <h1 className="text-2xl font-bold text-gray-900">Edytuj aktualność</h1>
        <p className="text-gray-500 mt-1">Edycja: {news.title}</p>
      </div>

      <NewsEditor tenantId={id} initialData={initialData} />
    </div>
  );
}
